    ...
    \begin{algorithmic}[1]
        \Procedure {Main}{\emph{TO planning problem}: $\mathcal{P} = (s_0,\omega,\mathcal{D})$}
            ...
            \While {$\mathcal{D}$ contains \emph{a method} $m$ with \emph{a between-constraint}}            
                \For {each \emph{compound task} $T$ in $subtasks(m)$ that is part of $\geq 1$ $between$ constraints}
                ...
                \For {each task $t$ in $subtasks(m)$}
                    ...
                    \State add $before(PropSymbols, t)$ to $m$
                    \State add $after(PropSymbols, t)$ to $m$
                \EndFor
                
                \State remove all $between$ constraints from $constraints(m)$
        ...
        \Procedure {SearchCompoundTask}{\emph{Compound task}: $T_{InputSymbols}$}
            \For {each \emph{method} $m$ with $T_{InputSymbols} = compound(m)$}
                ...
                \For {each \emph{compound task} $T$ in $subtasks(m)$}
                    ...
                    \EndIf
                    \State swap all occurrences of $T$ with $T_{PropSymbols \cup InputSymbols}$ in \emph{a method} $m$
                \EndFor
